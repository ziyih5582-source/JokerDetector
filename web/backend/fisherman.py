...
from profiles import ai_error_detail, ai_request_options, redact

MAX_TURNS = 40                     # 单次提交携带的历史条数上限
...
def stream_reply(analyzer, payload, meta):
...
def install_routes(app, analyzer, store_factory):
    """store_factory 指向 profile_routes.get_store，复用同一个本机档案库。"""
    router = APIRouter(prefix="/api/fisherman", tags=["问钓翁"])

    def load_profile(contact_id):
        try:
            return store_factory().get(contact_id)
        except KeyError:
    ...
    def chat(body: ChatInput):
        if analyzer.client is None:
            raise HTTPException(400, "尚未配置云端 AI。请先到「墨设」填好 API Key、模型 ID 与地址，并点「验证调用」")
        ...
        return StreamingResponse(
            stream_reply(analyzer, payload, meta),
