# -*- coding: utf-8 -*-
"""
Joker Detector 桌面版（Tkinter GUI）—— 早期实现，已不在维护。

保留全部 UI 模块：图片展示、数据网格、控制按钮与导出报告。
作为历史版本放在 legacy/ 下；现役实现是 backend/ + frontend/ 的 Web 版。
"""
...
# ================== 基础路径配置 ==================
# 本文件住在 legacy/desktop/，媒体与参考文本都在项目根的 assets/ 与 docs/ 下
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
PROJECT_ROOT = os.path.dirname(os.path.dirname(BASE_DIR))
PICTURES_DIR = os.path.join(PROJECT_ROOT, "assets", "pictures")
MUSIC_DIR = os.path.join(PROJECT_ROOT, "assets", "music")
REFERENCE_DIR = os.path.join(PROJECT_ROOT, "docs", "reference")
...
class JokerAnalyzer:
    def __init__(self, api_key=None, base_url=None, model=None):
        ...
        self.ai_model = model or MODEL
        self._init_ai()

    def _init_ai(self):
        if not self.api_key or self.api_key.startswith("sk-xxx"): return
        try:
            self.client = OpenAI(api_key=self.api_key, base_url=self.base_url, http_client=httpx.Client(trust_env=False))
        except: pass

    def update_ai_config(self, api_key, base_url=None, model=None):
        """运行时更新 AI 配置"""
        if not api_key or len(api_key.strip()) < 8:
            self.client = None
            return False
        self.api_key = api_key.strip()
        if base_url: self.base_url = base_url.strip()
        if model: self.ai_model = model.strip()
        try:
            self.client = OpenAI(api_key=self.api_key, base_url=self.base_url, http_client=httpx.Client(trust_env=False))
            return True
        except Exception:
...
class JokerDetectorGUI:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🃏 Joker Detector - 桌面版")
        ...
        # 加载本地知识库文件
        self.text_theory = _load_text("theory.txt")
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="智能情感分析终端", style='Subtitle.TLabel').pack(anchor='w', pady=(4, 0))
    ...
    def _build_left_panel(self, parent):
        ...
        self.ai_status_label = ttk.Label(info_frame, text="⚠️ AI 未连接（仅算法模式）", style='Card.TLabel', foreground=self.colors['warning'], font=(CN_FONT, 10))
        self.ai_status_label.pack(anchor='w', pady=(5, 5))
        ttk.Button(info_frame, text="⚙️ AI 配置", command=self._show_ai_config, style='Small.TButton').pack(anchor='w', pady=(0, 10))
    ...
    def _refresh_ai_status(self):
        if self.analyzer.client:
            self.ai_status_label.configure(text="✅ AI 已连接", foreground=self.colors['success'])
        else:
            self.ai_status_label.configure(text="⚠️ AI 未连接（仅算法模式）", foreground=self.colors['warning'])
    ...
    def _show_ai_config(self):
        ...
        ttk.Label(frame, text="配置 OpenAI 兼容 API", style='Accent.TLabel').pack(anchor='w', pady=(0, 15))

        ttk.Label(frame, text="API Key：", style='Card.TLabel').pack(anchor='w')
        key_entry = ttk.Entry(frame, width=60, show="*")
        key_entry.pack(fill='x', pady=(2, 10))
        key_entry.insert(0, self.analyzer.api_key if self.analyzer.api_key else "")

        ttk.Label(frame, text="Base URL（可选）：", style='Card.TLabel').pack(anchor='w')
        url_entry = ttk.Entry(frame, width=60)
        url_entry.pack(fill='x', pady=(2, 10))
        url_entry.insert(0, self.analyzer.base_url)

        ttk.Label(frame, text="模型名称（可选）：", style='Card.TLabel').pack(anchor='w')
        model_entry = ttk.Entry(frame, width=60)
        model_entry.pack(fill='x', pady=(2, 10))
        model_entry.insert(0, self.analyzer.ai_model)

        msg_label = ttk.Label(frame, text="", style='Card.TLabel')
        msg_label.pack(anchor='w', pady=(5, 5))

        def do_save():
            key = key_entry.get().strip()
            if not key:
                msg_label.configure(text="❌ API Key 不能为空", foreground=self.colors['warning'])
                return
            ok = self.analyzer.update_ai_config(key, url_entry.get().strip(), model_entry.get().strip())
            self._refresh_ai_status()
            if ok:
                msg_label.configure(text="✅ AI 连接成功！", foreground=self.colors['success'])
                popup.after(1000, popup.destroy)
            else:
                msg_label.configure(text="❌ 连接失败，请检查配置", foreground=self.colors['warning'])

        ttk.Button(frame, text="保存并连接", command=do_save, style='Accent.TButton').pack(anchor='w', pady=(5, 0))
