...
if use_ai:
    ...
    prompt = (
        '你是心理学与情感关系分析专家。请严格按照以下规则，分析输入的聊天记录，计算"恋爱小丑值"（0-100分，越高代表单方面过度付出、情感低位越严重）。\n\n'
        '规则依据：\n'
        '1. Bowlby依恋理论（痴迷型/焦虑型依恋）\n'
        '2. Gottman关系模型（末日四骑士、积极比例）\n'
        '3. Rusbult投资模型（情感-时间-精力对等性）\n\n'
        '七维度扣分体系（总分100，每维度独立扣分，累计上限100）：\n\n'
        '【维度一】发起回应失衡(权重20)：一方发起对话超70%扣4分，超80%扣7分，超90%扣10分；主动填补冷场扣4分；连发3条未回应扣4分。\n'
        '【维度二】痴迷型依恋(权重18)：未回复时连续追问/电话轰炸扣5-6分；宣泄情绪扣3分/次；故意不回复测试对方扣5分；频繁问"你是不是不爱我了"扣3分/次。\n'
        '【维度三】情感投资对等性(权重15)：单方面请客/转账扣3分/次；牺牲型表达扣3分/次；情感劳动单边化扣4分。\n'
        '【维度四】戈特曼四骑士(权重14)：批评、蔑视扣3-4分/次；防御扣2分/次；冷战(超8小时不回)扣3分/次。\n'
        '【维度五】自我价值矮化(权重12)：过度道歉扣2分/次；自贬句式扣3分/次；乞求句式扣3分/次。\n'
        '【维度六】积极消极比例(权重11)：积极词少于30%扣3分，消极词多于40%扣4分，正面/负面比<2:1扣3分。\n'
        '【维度七】权力动态失衡(权重10)：决策全由对方定扣3分；威胁控制扣4分/次；优先度沦陷扣3分。\n\n'
        '计算步骤：\n'
        '1. 逐项检测聊天记录，记录每个扣分项目的发生次数。\n'
        '2. 按权重累计扣分（每项扣分不超过该维度上限）。\n'
        '3. 最终得分 = 100 - 总扣分，若低于0则取0。\n\n'
        '输出要求：\n'
        '- 只输出一个数字（0-100），以及一行最核心的扣分原因（不超过15字）。\n'
        '- 示例输出：72  焦虑型追问+单方面付出\n\n'
        '请开始分析以下聊天记录：\n\n'
        f"{transcript}"
    )
    try:
        response = client.chat.completions.create(
            model=ai_model,
            messages=[
                {"role": "system", "content": "你只输出一个0到100之间的数字，以及一行扣分原因（不超过15字），格式如：72  焦虑型追问+单方面付出"},
        ...
        if match:
            ...
            jokernum_ai = max(0.0, min(100.0, jokernum_ai))
...
if use_ai:
    ...
    guidance_prompt = (
        ...
        "请从【自己】的视角出发，提供一段详细的Jokernum得分分析报告，包含情感疏导，要求：\n"
        "1. 先安抚情绪，认可对方的正常情感需求；\n"
        "2. 再客观指出这段关系中可能存在的自我感动、边界模糊等问题；\n"
        "3. 给出 2~3 条具体、可执行的心理调节或行为改变建议；\n"
        "4. 语气温柔、不带评判，像朋友聊天一样。\n\n"
        "https://chat.deepseek.com/share/6vdvmx9l3ws70ana9y根据这个网页的内容给出得分分析"
        f"{transcript}"
    )
    try:
