...
const BASE_URL = ''; // Same-origin API; also works when the local port changes.
...
async function checkHealth() {
    try {
        ...
        const statusText = data.ai_available
            ? `${data.ai_verified ? '✅ AI 调用已验证' : '⚙️ AI 已配置，待测试'} (${data.ai_model})`
            : '⚠️ 仅算法模式';
        document.getElementById('footer-status').textContent = statusText;
        // 更新 AI 状态指示灯
        const dot = document.getElementById('ai-status-dot');
        if (data.ai_available && data.ai_verified) {
...
async function runDemo(demoId) {
    ...
    try {
        const useAI = document.getElementById('analysis-use-ai').checked;
        const result = await apiCall(`/api/analyze/demo/${demoId}?cloud_consent=${useAI}`, { method: 'POST' });
...
function setupUpload() {
    ...
    selectDiv.addEventListener('click', event => event.stopPropagation());
    ...
    document.getElementById('analyze-btn').addEventListener('click', async () => {
        ...
        try {
            ...
            const resp = await fetch(`${BASE_URL}/api/analyze/upload?self_index=${selfIndex}&cloud_consent=${useAI}`, {
                method: 'POST',
                body: formData
            });
            if (!resp.ok) {
...
async function handleFileSelect(file) {
    ...
    document.getElementById('speaker-0').textContent = '发言最多者';
    document.getElementById('speaker-1').textContent = '另一位发言者（按实际姓名选择请用联系人档案）';
}
...
function displayResult(data) {
    ...
    if (data.guidance) {
        ...
        guidanceSection.classList.remove('hidden');
        document.getElementById('guidance-title').textContent = 'AI 情感长文暂未生成';
        document.getElementById('guidance-content').textContent = data.guidance_error + '。聊天统计已保留，可修复配置后重新分析。';
    } else {
...
function renderChatReplay(messages) {
    ...
    function showBatch() {
        ...
        batch.forEach(msg => {
            ...
            div.innerHTML = `
                <div class="chat-msg-speaker">${escapeHtml(msg.speaker)}</div>
...
function setupAIConfig() {
    ...
    saveBtn.addEventListener('click', async () => {
        ...
        if (!model) {
            msgEl.textContent = '请输入服务商提供的完整模型 ID';
            msgEl.className = 'ai-config-msg error';
        ...
        try {
            ...
            if (!resp.ok) throw new Error(typeof data.detail === 'string' ? data.detail : '配置格式有误');
            if (data.success) {
                document.getElementById('ai-api-key').value = '';
                msgEl.textContent = `配置已保存：${data.model}。请点击“测试已保存配置”验证实际调用。`;
                msgEl.className = 'ai-config-msg';
                document.getElementById('ai-status-dot').classList.remove('active');
                document.getElementById('footer-status').textContent =
                    `⚙️ AI 已配置，待测试 (${data.model})`;
            } else {
                msgEl.textContent = '❌ 配置未保存，请检查 API Key 和地址';
                msgEl.className = 'ai-config-msg error';
            }
        } catch (err) {
    ...
    testBtn.addEventListener('click', async () => {
        ...
        msgEl.textContent = '正在发送固定的虚构示例，验证已保存的模型配置…';
        msgEl.className = 'ai-config-msg';
        try {
            const data = await apiCall('/api/config/test', { method: 'POST' });
            msgEl.textContent = data.success ? `✅ ${data.message}（${data.model}）` : `❌ ${data.error.message}`;
            msgEl.className = 'ai-config-msg' + (data.success ? '' : ' error');
            await checkHealth();
        } catch (err) {
            msgEl.textContent = '❌ 测试失败：' + err.message;
            msgEl.className = 'ai-config-msg error';
        } finally {
            saveBtn.disabled = false;
            testBtn.disabled = false;
            testBtn.textContent = '测试已保存配置';
        }
    });
}
