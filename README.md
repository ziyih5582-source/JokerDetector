...
导航新增一处**「小丑鉴定所」**（朱砂底 + 跳动红点，很好认），首页还把一尊**经典马戏团小丑全身像固定在整屏最右侧**当入口（窄屏自动隐藏，导航入口仍在）。它回答一个很具体的问题：**这段关系里，我是不是把自己活成了小丑？**

**1. 「小丑」是一种关系状态，不是骂人。**
指明知对方不够喜欢自己，仍靠单方面付出、自我感动和幻想维持关系——把冷淡当考验、把零星回应当希望，最后期待落空。**认真喜欢一个人不丢人**；只有长期不对等、失去边界、明知不可为而为之才算。

**2. 六征判定，本地完成。**
把上面这套定义拆成六个可量化的特征：**单向性、自我感动、幻想性、边界失守、不接受拒绝、事后自嘲**。判定规则刻意保守以防误伤：**单向性命中 + 其余五项至少命中一项 + 综合分 ≥ 45**，才会判为小丑。全程在本机跑，不调用云端 AI，也不写任何档案库。

**3. 六种类型 + 专属形象。**
在原有四型（殉道 / 镜像 / 弄臣 / 幻恋）之外新增两型：**守候型**（被拒绝仍不肯放手）与**单向型**（关系全靠你推进）。每种类型配一套纯 SVG 形象，配色取自该类型主色：殉道型捧心含泪、镜像型半脸成镜像、弄臣型铃铛帽大笑、幻恋型做梦飘星、守候型捧着将熄的烛火、单向型独自拉单向箭头。

**4. 判定后的呈现。**
判定为小丑时，**全屏会弹出该类型的专属小丑在屏幕上快速弹跳游走**（数量随浓度 7～18 个、铺满整屏，随机冒出自嘲气泡），同时弹出**诊断弹窗**：标题是「确诊：某某型」，**右上角放着同款形象的静态小丑**，下面逐条列出六征的命中情况与依据，收尾是一句「我知道了，先爱自己」。

**5. 鉴定所页面：六征上方立一尊全身马戏团小丑。**
`小丑鉴定所` 页右侧「六征」列表的上方，立着一尊全身马戏团小丑（纯 SVG，带轻微上下浮动），与首页最右侧的入口小丑同款。

**6. 鉴定所也支持「导入长截图」。**
鉴定所的聊天输入口除粘贴文本、导入 Excel 外，新增**导入长截图（可多选）**，与「谈心分析」共用同一套本机 OCR：识别出的结果排成一张可逐条校对/修改的表格（文字默认折叠、图片与表情常展开），改完再点「开鉴」。OCR 依赖同样是可选的，未安装时该入口置灰并提示安装命令。

---

## 上一版更新：微信长截图导入（阶段 1 + 2 + 3）
...
**2. 页面与按钮全部改成大白话。**
导航共七个去处：**首页 · 谈心分析 · 找钓翁聊聊 · 人物档案 · 心理学依据 · 使用说明 · 设置**（小丑鉴定所上线后为八处），另有一个不在导航里的结果页**「分析结果」**（保留「水纹」这个别称）。按钮顺势改名：「投食 → 洒入水中」变「谈心分析 → 开始分析」，「择卷 → 录名 → 抹去此人」变「导入 Excel → 新建档案 → 删除此人」，「拓一张纸」变「导出为图片」，「再投一次」变「再分析一段」，「静水」变「暂停湖面动画」，「配乐」变「背景音乐」，「示例 · 初 / 示例 · 变」变「填入示例一 / 填入示例二」。钓翁这个人设保留，只是从「潭边」挪到了「思源湖畔垂钓多年的老者」。
...
**4. 后端文案随之对齐。**
未配置云端 AI 时的提示现在指向「设置」页（不再叫「墨设」），`backend/tests/test_fisherman.py` 的断言也一并改成「设置」；测试仍是 **112 项**，全部通过。（小丑鉴定所上线后新增 10 项测试；装齐 OCR 可选依赖后为 **122 项通过**、不装则 9 项跳过，见「测试与检查」。）
...
想做一次快速自测：点导航里的「小丑鉴定所」（首页的全身小丑也是入口）→ 粘贴或导入聊天 → 「辨认说话的人」→ 「开鉴」。它只走本地六征判定，绿色通道、不调云端；判定为小丑时会弹出类型弹窗并把该类型的小丑洒满屏幕。
...
导航里的「小丑鉴定所」是一处独立的本地自测，回答「这段关系里我是不是把自己活成了小丑」。它不经过 `/api/analyze/unified`，直接走 `POST /api/analyze/joker/detect`，**纯本地、不调云端、不写档案**。

**六征**（对应定义里的六个特征，权重与阈值都在 `backend/app/services/analyzer.py`）：

| 征状 | 说明 | 主要依据 |
| --- | --- | --- |
| 单向性 | 关系全靠你推进 | 消息比例、最长连发比值 |
| 自我感动 | 付出换爱、随叫随到 | 付出型话术 + PLD |
| 幻想性 | 把冷淡解读成还有机会 | 脑补类词汇 + SSDT |
| 边界失守 | 为对方牺牲、失去底线 | 牺牲／托付型表达 |
| 不接受拒绝 | 被拒后还在等 | 拒绝后继续发送比例 + 挽留词 |
| 事后自嘲 | 「小丑竟是我自己」 | 自贬词汇 |

**判定规则（刻意保守，防误伤）**：`单向性命中 且 其余五项至少命中一项 且 综合分 ≥ 45` 才判小丑；综合分为算法分 + 命中数 × 6（上限 100），出现语音/通话再 ×0.95。类型取命中征状里得分最高的一项；六型为**殉道型 / 镜像型 / 弄臣型 / 幻恋型 / 守候型 / 单向型**。

**呈现**：判定为小丑时，全屏弹出该类型的专属小丑（`frontend/js/clown.js` 里的纯 SVG 形象，按浓度出 7～18 个、快速弹跳游走并随机冒气泡），同时弹出诊断弹窗，右上角放着同款静态形象，下方逐条列出六征命中情况。入口方面，导航里有一处「小丑鉴定所」，首页另把一尊经典马戏团小丑全身像固定在最右侧（窄屏自动隐藏）。
