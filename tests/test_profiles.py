...
from profiles import ProfileStore, extract_ai, extract_local, prepare_messages, ai_error_detail
...
def test_cloud_opt_in_redacted_prompt_and_duplicate_skips_cloud(client, monkeypatch):
    ...
    monkeypatch.setattr(main.analyzer, "client", ai)
...
def test_ai_invalid_json_falls_back_visibly(client, monkeypatch):
    monkeypatch.setattr(main.analyzer, "client", FakeAI("not-json"))
...
def test_config_test_uses_fictional_data_and_distinguishes_verification(client, monkeypatch):
    ...
    ai = FakeAI('{"items":[]}')
    monkeypatch.setattr(main.analyzer, "client", ai)
    monkeypatch.setattr(main.analyzer, "ai_verified", False)
    response = client.post('/api/config/test')
    assert response.json()["success"] and main.analyzer.ai_verified
    ...
    response = client.post('/api/config/test')
    assert not response.json()["success"] and not main.analyzer.ai_verified
...
def test_security_headers_cross_origin_and_legacy_local_only(client, monkeypatch):
    ...
    ai = FakeAI("should not run"); monkeypatch.setattr(main.analyzer, "client", ai)
...
def test_failed_long_report_keeps_stats_and_shows_safe_reason(client, monkeypatch):
    class BrokenAI(ReportAI):
        def create(self, **kwargs):
            ...
            return super().create(**kwargs)
    monkeypatch.setattr(main.analyzer, 'client', BrokenAI(''))
