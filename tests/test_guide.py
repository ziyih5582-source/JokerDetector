...
from analyzer import METRIC_WEIGHTS, SCORE_MIDPOINT, SCORE_STEEPNESS, VOICE_PENALTY, JOKER_TYPES
...
def test_guide_thresholds_are_the_ones_actually_used(client):
    ...
    assert [(l["min"], l["key"]) for l in levels] == main.LEVEL_THRESHOLDS
    for minimum, key in main.LEVEL_THRESHOLDS:
        assert main._get_level(minimum) == key              # 阈值处属于这一档
        if minimum > 0:
            assert main._get_level(minimum - 0.01) != key    # 差一点就掉到下一档
    assert main._get_level(100) == "confirmed"
    assert main._get_level(0) == "healthy"
